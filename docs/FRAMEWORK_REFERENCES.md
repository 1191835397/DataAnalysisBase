...
近期已落地：`dab config validate`、`dab doctor`、`dab status --json`、`dab plan sync-market`、`dab sync market` dry-run、provider 本地健康检查、mock provider 同步测试。
...
| ECC 治理/诊断形态 | `dab config validate`、`dab doctor`、`dab status --json`、`dab plan sync-market`、`dab sync market` dry-run、provider 本地健康检查、`/api/v1/system/status` | 不接管多 harness 配置，不引入 hooks runtime |
...
- Phase A 先提供最小诊断面：`dab config validate`、`dab doctor`、`dab status --json`、`dab plan sync-market`、`dab sync market` dry-run、provider 本地健康检查、`/api/v1/system/status`。
