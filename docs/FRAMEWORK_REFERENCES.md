...
近期已落地：`dab config validate`、`dab doctor`、`dab status --json`、`dab plan sync-market`、mock provider 同步测试。
...
| ECC 治理/诊断形态 | `dab config validate`、`dab doctor`、`dab status --json`、`dab plan sync-market`、`/api/v1/system/status` | 不接管多 harness 配置，不引入 hooks runtime |
...
- Phase A 先提供最小诊断面：`dab config validate`、`dab doctor`、`dab status --json`、`dab plan sync-market`、`/api/v1/system/status`。
- 已补齐 mock provider 同步测试。
- 后续补齐 Web System Status 页面和真实 provider adapter。
