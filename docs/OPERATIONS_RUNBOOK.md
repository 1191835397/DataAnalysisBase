...
- 当前可用同步能力：API 触发后台市场同步、取消、持久化任务、历史分页、最近失败统计、任务详情、结构化运行日志、运行 artifact、重试入口、本地行业映射文件回填最新快照、独立停牌提醒/清单补充 `is_suspended`
